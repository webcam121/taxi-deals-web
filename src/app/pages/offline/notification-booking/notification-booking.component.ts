...
@Component({
  ...
  styleUrls: ['./notification-booking.component.css']
})
export class NotificationBookingComponent implements OnInit {
  ...
  saveNotification(data){
    ...
    const input = {
      ...
      category: this.bookingSMSForm.value['category'],
      "coupen": "string",
      "des": this.bookingSMSForm.value.message,
      "driverId": "string",
      "isAndroid": "string",
      "km": "string",
      latitude: this.lat2,
...
if(validation){
  this._offlineService.sendTripToDeviceUserAll(input).subscribe((res: any) => {
    if(res.status) {
      this._alertService.success(res.message);
    }
  },
