...
export class OfflineBookingComponent implements OnInit {
  ...
  public lat = 11.0168445;
  public lng = 76.9558321;
  zoom = 12;
  dlng: any;
  dlat: any;
  lat1 = 11.0168445;
  ...
  destination(response) {
   //  // // console.log("ride response",response);
    const pointA = new google.maps.LatLng(this.lat1, this.lng1),
      ...
      this.onlineDatas.map(x => {
        // tslint:disable-next-line:no-unused-expression
        new google.maps.Marker({
          position: new google.maps.LatLng(x.lat, x.lng),
          title: 'Name: ' + x.name + ',CarType:' + x.cartype + ',PhoneNumber:' + x.phoneNumber + ',status:' + x.status ,
          icon: {
            url: this.imageStatus(x),
  ...
  getResult(inputRequest) {
    ...
    this._offlineService.getSearchResult(inputRequest).subscribe(
      (res) => {
        ...
        this.bookMyTaxi = [];
        this.bookMyTaxi = res.data;
        this.bookMyTaxi.map(e => {
          const onlineData = {
            lat: parseFloat(e.latitude),
            lng: parseFloat(e.longitude),
            id: e.driverId,
            imageUrl: this.imageStatus(e),
            name: e.name,
            cartype: e.carType,
